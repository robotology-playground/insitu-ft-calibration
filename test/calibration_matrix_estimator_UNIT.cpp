...
int main()
{
    InSituFTCalibration::CalibrationMatrixEstimator calibration_matrix_estimator;
    ...
    Eigen::Matrix<double,6,6> calibration_matrix = Eigen::Matrix<double,6,6>::Random();
    Eigen::Matrix<double,6,6> shape_matrix = calibration_matrix.inverse();
    ...
    for(int dataset=0; dataset < n_datasets; dataset++ ) 
    {
        ...
        int dataset_id = calibration_matrix_estimator.addCalibrationDataset(dataset_name,masses[dataset],wrapVec(coms[dataset]));
        double return_mass;
    ...
    std::string dummy;
    custom_assert_true(calibration_matrix_estimator.computeCalibrationMatrixEstimation(dummy),"Calibration matrix computation failed");
    
    Eigen::Matrix<double,6,6>  estimated_calibration_matrix;
    custom_assert_true(calibration_matrix_estimator.getEstimatedCalibrationMatrix(wrapMat(estimated_calibration_matrix)),"Calibration matrix retrieval failed");
    
    std::cout << "[INFO] true      calibration matrix:  \n" << calibration_matrix << std::endl;
    std::cout << "[INFO] estimated calibration matrix:  \n" << estimated_calibration_matrix << std::endl;
    std::cout << "[INFO] true-estimated calibration mat:\n" << calibration_matrix-estimated_calibration_matrix << std::endl;
    ...
    for(int i=0; i < 6; i++ )
    {
        for(int j=0; j < 6; j++ ) 
        {
            custom_assert_close(calibration_matrix(i,j),estimated_calibration_matrix(j),tol,"Difference between true calibration matrix and estimated one");
        }
    }
